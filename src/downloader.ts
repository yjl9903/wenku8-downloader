...
export async function downloadNovel(novelId: number, options: CommandOptions) {
  try {
    ...
    if (novel.catalogueUrl) {
      ...
      for (const volume of volumes) {
        ...
        if (options.epub) {
          ...
          if (!fs.existsSync(path.join(process.cwd(), options.outDir, novel.novelName, volumeNameWithIndex))) {
            fs.mkdirSync(path.join(process.cwd(), options.outDir, novel.novelName, volumeNameWithIndex, '插图'), {
              recursive: true,
            });
          }
        }
          ...
          for (const { chapterIndex, chapterTitle, chapterUrl } of chapters) {
            scheduler.add(async () => {
              try {
                ...
                const { content, images } = await retryFn(async () => downloadChapter(chapterUrl, options));
                const paths: string[] = [];
                if (images.length) {
                    ...
                    if (!options.epub) {
                      scheduler.add(async () => {
                        try {
                          ...
                          const res = await retryFn(async () => axios.get(imageUrl, { responseType: 'arraybuffer' }));
                          ...
                          errorTimes++;
                          console.log(chalk.red(`${volume.name}-${chapterTitle}-${imagePath}下载失败`));
                          return appendFile(
                ...
                if (options.epub) {
                  epubOptions.content[volume.index].content[chapterIndex] = `<h1>${chapterTitle}</h1>` + content;
      ...
      console.log(chalk.bold.green(`『 ${novel.novelName} 』` + '下载完成!' + `总共用时${minutes}分${seconds}秒`));
      if (errorTimes) {
  ...
  if ($('#contentmain span').first().text().trim() == 'null') {
    ...
    try {
      ...
      if (error.message.indexOf('404') !== -1) {
        const res = await axios.get(`http://dl.wenku8.com/packtxt.php?aid=${v.slice(-2)[0]}&vid=${v.slice(-1)[0]}`, {
...
export async function search(
  ...
  const $ = await fetch(`https://www.wenku8.net/modules/article/search.php?searchtype=${type}&searchkey=${searchKey}`);
  const catalogueUrl = $('#content')
    .children()
    .first()
    .children()
